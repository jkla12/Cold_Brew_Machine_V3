 ...
 * ************************************************************************
 */
uint8_t MQTT_PublishStatus(const char *payload)
{
    int bufferFound = -1;
    ...
    {
        log_e("MQTT is not connected, cannot publish status");
        return 0;
    }
    ...
    snprintf(atCommand, sizeof(atCommand), "AT+MQTTPUBRAW=\"%s\",1,0,%zu",
             deviceInfo.data.MQTT.pubTopic, payloadLen);
...
{
    if (success) {
        log_d("MQTT message published successfully");
    } else {
        log_e("MQTT message publish failed: %s", response);
    }

    // 标记缓冲区为空闲
    if (currentCmd.needRawSend && currentCmd.rawData && currentCmd.bufferIndex < PAYLOAD_BUFFER_COUNT) {
...
/**
 * ************************************************************************
 * @brief 发送AT命令，支持RAW数据
 *
 * @param[in] command 命令结构体指针
 *
 * @return 是否成功发送命令
 *
 * @version 1.0
...
uint8_t AT_SendCommandRaw(AT_Command_TypeDef *command)
{
    if (!WIFI.isInitialized)
    {
        log_e("WIFI module not initialized");
        return 0;
    }

    // 等待上一条命令完成，但不阻塞过长时间
    //   uint32_t startTime = Timer.systemTime;
    if (currentCmd.state != AT_IDLE)
    {
        // 如果上一个命令还在处理中，返回失败
        if (Timer.systemTime - currentCmd.startTime < 30000)
        { // 如果上一条命令在合理时间内
            log_e("Previous AT command is still processing");
            return 0;
        }
        else
        {
            // 强制重置命令状态，可能上一条命令已经卡死
            log_w("Forcing command state reset, previous command may be stuck");
            currentCmd.state = AT_IDLE;
        }
    }

    // 复制命令结构体内容
    memcpy(&currentCmd, command, sizeof(AT_Command_TypeDef));
